...
impl Window {
    ...
    pub fn draw(&mut self, cx: &mut App) -> ArenaClearNeeded {
        ...
        let frame_start = TRACE
            .get_or_init(|| std::env::var_os("GAMESYNC_FRAME_TRACE").is_some())
            .then(Instant::now);
        ...
        if let Some(start) = frame_start {
            eprintln!("[card-frame] cpu_us={}", start.elapsed().as_micros());
        }
        ArenaClearNeeded
    }
    ...
    pub(crate) fn paint_card_layer(
        ...
        let parent = std::mem::take(&mut self.next_frame.scene);
        // Capture the entire face even when its outer row is partly outside the viewport.
        self.content_mask_stack.push(ContentMask { bounds });
        paint(self);
        self.content_mask_stack.pop();
        let captured = std::mem::replace(&mut self.next_frame.scene, parent);
        let scaled = bounds.scale(self.scale_factor());
        let offset = scaled.origin.map(|value| ScaledPixels(-value.0));
        let mut local = Scene::default();
        for operation in captured.paint_operations {
            match operation {
                crate::scene::PaintOperation::Primitive(mut primitive) => {
                    primitive.translate(offset);
                    local.insert_primitive(primitive);
                }
                crate::scene::PaintOperation::StartLayer(mut bounds) => {
                    bounds.origin += offset;
                    local.push_layer(bounds);
                }
                crate::scene::PaintOperation::EndLayer => local.pop_layer(),
            }
        }
        local.finish();
        self.next_frame.scene.insert_primitive(crate::PaintSurface {
