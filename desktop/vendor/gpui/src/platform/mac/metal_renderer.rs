...
use card_renderer::CardRenderer;
use cocoa::{
    base::{NO, YES},
...
impl MetalRenderer {
    pub fn new(instance_buffer_pool: Arc<Mutex<InstanceBufferPool>>) -> Self {
        ...
        let cards = CardRenderer::new(&device);
        Self {
            cards,
            device,
            layer,
    ...
    pub fn draw(&mut self, scene: &Scene) {
        ...
        loop {
            ...
            match command_buffer {
                ...
                Err(err) => {
                    self.cards.discard_unsubmitted();
                    log::error!(
    ...
    fn draw_primitives(
        ...
        self.encode_scene(
            ...
            viewport_size,
            command_buffer,
            if self.layer.is_opaque() { 1. } else { 0. },
        )?;
        instance_buffer.metal_buffer.did_modify_range(NSRange {
            location: 0,
            length: offset as NSUInteger,
        });
        self.cards.on_submit(command_buffer);
    ...
    fn encode_scene(
        ...
        let mut command_encoder =
            new_command_encoder(command_buffer, target, viewport_size, |color_attachment| {
            ...
            let ok = match batch {
                ...
                PrimitiveBatch::Surfaces(surfaces) => {
                    ...
                    for surface in surfaces {
                        if let Some(card) = &surface.card {
                            if !self.cards.cached(surface) {
                                command_encoder.end_encoding();
                                if let Some(texture) = self.cards.allocate(surface, &self.device) {
                                    let dimensions = size(
                                        DevicePixels(texture.width() as i32),
                                        DevicePixels(texture.height() as i32),
                                    );
                                    // Reserve the parent's allocation before any nested artwork mask.
                                    self.cards.store(surface, texture.clone());
                                    self.encode_scene(
                                        &card.scene,
                                        instance_buffer,
                                        instance_offset,
                            ...
                            self.cards.draw(surface, viewport_size, command_encoder);
                        } else {
                            success &= self.draw_surfaces(
