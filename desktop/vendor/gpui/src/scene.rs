...
impl Primitive {
    ...
    pub(crate) fn translate(&mut self, offset: Point<ScaledPixels>) {
        ...
        match self {
            Self::Shadow(p) => shift!(p),
            Self::Quad(p) => shift!(p),
            Self::Underline(p) => shift!(p),
            Self::MonochromeSprite(p) => shift!(p),
            Self::PolychromeSprite(p) => shift!(p),
            Self::Surface(p) => shift!(p),
            Self::Path(p) => {
                shift!(p);
                p.start += offset;
                p.current += offset;
                for v in &mut p.vertices {
                    v.xy_position += offset;
                    v.content_mask.bounds.origin += offset;
                }
            }
        }
    }
